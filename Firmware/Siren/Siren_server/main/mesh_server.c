...
static void prov_complete(uint16_t net_idx, uint16_t addr, uint8_t flags, uint32_t iv_index) {
   ...
   esp_ble_mesh_model_subscribe_group_addr(addr,CID_ESP,ESP_BLE_MESH_CUSTOM_SENSOR_MODEL_ID_SERVER,0xC000);
...
static void parse_received_data(esp_ble_mesh_model_cb_param_t *recv_param, model_encoder_data_t *parsed_data) {
    ...
    ESP_LOGW("PARSED_DATA", " Temperature = %.2f", parsed_data->temperature);
     ESP_LOGW("PARSED_DATA", " Humidity = %.2f", parsed_data->humidity);
      ...
      ESP_LOGW("PARSED_DATA", " Siren %d", parsed_data->siren_onoff);
