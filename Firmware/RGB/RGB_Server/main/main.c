...
static void read_received_items(void *arg) {
    ...
    while (1) {
        ...
        if (xQueueReceive(ble_mesh_received_data_queue, &_received_data, 1000 / portTICK_RATE_MS) == pdPASS) {
            ESP_LOGI(TAG, "red color %d", _received_data.red);
             ESP_LOGI(TAG, "green color %d", _received_data.green);
              ESP_LOGI(TAG, "blue color %d", _received_data.blue);
             rgb_led_set_color(_received_data.red, _received_data.green, _received_data.blue);
             vTaskDelay(500/portTICK_PERIOD_MS);
              rgb_led_set_color( (_received_data.red + 10)%255 , (_received_data.green + 10) %255, _received_data.blue);
