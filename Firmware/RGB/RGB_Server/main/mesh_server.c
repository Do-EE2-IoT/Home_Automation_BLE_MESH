...
static esp_ble_mesh_model_op_t custom_sensor_op[] = {
    ESP_BLE_MESH_MODEL_OP(ESP_BLE_MESH_CUSTOM_RGB_MODEL_OP_GET, 0),  // OP_GET no minimo 0 bytes
    ESP_BLE_MESH_MODEL_OP(ESP_BLE_MESH_CUSTOM_RGB_MODEL_OP_SET, 4),  // OP_SET no minimo 4 bytes
    ESP_BLE_MESH_MODEL_OP_END,
};

static model_encoder_data_t _server_model_state = {
    .red = 0,
    .green = 0,
    .blue = 0
};
...
static esp_ble_mesh_model_t custom_models[] = {
    ESP_BLE_MESH_VENDOR_MODEL(CID_ESP, ESP_BLE_MESH_CUSTOM_RGB_MODEL_ID_SERVER, 
...
static void prov_complete(uint16_t net_idx, uint16_t addr, uint8_t flags, uint32_t iv_index) {
   ...
   esp_ble_mesh_model_subscribe_group_addr(addr,CID_ESP,ESP_BLE_MESH_CUSTOM_RGB_MODEL_ID_SERVER,0xC000);
...
static void ble_mesh_config_server_cb(esp_ble_mesh_cfg_server_cb_event_t event,
    ...
    if (event == ESP_BLE_MESH_CFG_SERVER_STATE_CHANGE_EVT) {
        ...
        ESP_LOGI(TAG, "undefine opcode 0x0%8"PRIx32, param->ctx.recv_op);
    }
}

static void ble_mesh_custom_sensor_server_model_cb(esp_ble_mesh_model_cb_event_t event,
                                                    esp_ble_mesh_model_cb_param_t *param) {
    model_encoder_data_t response;
    esp_err_t err;
    switch (event) {
        ...
        case ESP_BLE_MESH_MODEL_OPERATION_EVT:
            switch (param->model_operation.opcode) {
                ...
                case ESP_BLE_MESH_CUSTOM_RGB_MODEL_OP_GET:
                 ...
                 ESP_LOGI(TAG, "OP_GET -- opcode 0x%06" PRIx32, param->model_operation.opcode);
                  ...
                  ESP_LOGW(TAG, "OP_GET --  opcode 0x%06"PRIx32 , param->model_operation.opcode);
                    ...
                    err = esp_ble_mesh_server_model_send_msg(param->model_operation.model, param->model_operation.ctx, 
                                    ESP_BLE_MESH_CUSTOM_RGB_MODEL_OP_STATUS, sizeof(response), (uint8_t *)&response);
                    if (err) {
                       // ESP_LOGE(TAG, "%s Cannot send opcode OPCODE 0x%06"PRIx32, __func__, ESP_BLE_MESH_CUSTOM_SENSOR_MODEL_OP_STATUS);
                    }
                break;

                case ESP_BLE_MESH_CUSTOM_RGB_MODEL_OP_SET:
                    response = *(model_encoder_data_t *)param->model_operation.model->user_data;
