...
static const esp_ble_mesh_client_op_pair_t custom_model_op_pair_SIREN[] = {
    {ESP_BLE_MESH_CUSTOM_SENSOR_MODEL_OP_GET, ESP_BLE_MESH_CUSTOM_SIREN_MODEL_OP_STATUS},
    {ESP_BLE_MESH_CUSTOM_SENSOR_MODEL_OP_SET, ESP_BLE_MESH_CUSTOM_SIREN_MODEL_OP_STATUS},
};
//* SENSOR opcode
...
static esp_ble_mesh_model_op_t custom_siren_op_SIREN[] = {
    ESP_BLE_MESH_MODEL_OP(ESP_BLE_MESH_CUSTOM_SIREN_MODEL_OP_STATUS, 2),
    ESP_BLE_MESH_MODEL_OP_END,
};
//* Definicao de fato dos opcodes aqui
                                ...
                                uint8_t adv_type, esp_ble_mesh_prov_bearer_t bearer)
{
    ...
    if (device_type == BUTTON)
    {
        ...
        // Open
        printf("\n");
        printf("Opening NVS to get clientID");
        ...
        // close
        nvs_close(my_handle);
        sprintf(data, "{\"type\":\"button\",\"uuid\":\"dddd%02x%02x%02x%02x%02x%02x0000000000000001\",\"mac\":\"%02x%02x%02x%02x%02x%02x\",\"clientID\":\"%s\"}",
                new_bluetooth_device_uuid[2], new_bluetooth_device_uuid[3], new_bluetooth_device_uuid[4], new_bluetooth_device_uuid[5],
                new_bluetooth_device_uuid[6], new_bluetooth_device_uuid[7], new_bluetooth_device_mac[0],
                new_bluetooth_device_mac[1], new_bluetooth_device_mac[2], new_bluetooth_device_mac[3],
                new_bluetooth_device_mac[4], new_bluetooth_device_mac[5], output);
        // ESP_LOGI(TAG,"my client ID read 1 is %s", output);
        // ESP_LOGI(TAG,"DATA is %s", data);
        vTaskDelay(500/ portTICK_PERIOD_MS);
        mqtt_client_publish("/device/detect", 1, data);
        free(data);
    }
    else if (device_type == RGB)
    {
        ...
        // close
        nvs_close(my_handle);
        sprintf(data, "{\"type\":\"rgb\",\"uuid\":\"dddd%02x%02x%02x%02x%02x%02x0000000000000003\",\"mac\":\"%02x%02x%02x%02x%02x%02x\",\"clientID\":\"%s\"}",
                new_bluetooth_device_uuid[2], new_bluetooth_device_uuid[3], new_bluetooth_device_uuid[4], new_bluetooth_device_uuid[5],
                new_bluetooth_device_uuid[6], new_bluetooth_device_uuid[7], new_bluetooth_device_mac[0],
                new_bluetooth_device_mac[1], new_bluetooth_device_mac[2], new_bluetooth_device_mac[3],
                new_bluetooth_device_mac[4], new_bluetooth_device_mac[5], output);
    ...
    else if (device_type == SIREN)
    {
        ...
        // close
        nvs_close(my_handle);
        sprintf(data, "{\"type\":\"siren\",\"uuid\":\"dddd%02x%02x%02x%02x%02x%02x0000000000000005\",\"mac\":\"%02x%02x%02x%02x%02x%02x\",\"clientID\":\"%s\"}",
                new_bluetooth_device_uuid[2], new_bluetooth_device_uuid[3], new_bluetooth_device_uuid[4], new_bluetooth_device_uuid[5],
                new_bluetooth_device_uuid[6], new_bluetooth_device_uuid[7], new_bluetooth_device_mac[0],
                new_bluetooth_device_mac[1], new_bluetooth_device_mac[2], new_bluetooth_device_mac[3],
                new_bluetooth_device_mac[4], new_bluetooth_device_mac[5], output);
    ...
    else if (device_type == ENCODER)
    {
        ...
        // close
        nvs_close(my_handle);
        sprintf(data, "{\"type\":\"encoder\",\"uuid\":\"dddd%02x%02x%02x%02x%02x%02x0000000000000002\",\"mac\":\"%02x%02x%02x%02x%02x%02x\",\"clientID\":\"%s\"}",
                new_bluetooth_device_uuid[2], new_bluetooth_device_uuid[3], new_bluetooth_device_uuid[4], new_bluetooth_device_uuid[5],
                new_bluetooth_device_uuid[6], new_bluetooth_device_uuid[7], new_bluetooth_device_mac[0],
                new_bluetooth_device_mac[1], new_bluetooth_device_mac[2], new_bluetooth_device_mac[3],
                new_bluetooth_device_mac[4], new_bluetooth_device_mac[5], output);
    ...
    else if (device_type == SENSOR)
    {
        ...
        // close
        nvs_close(my_handle);
        sprintf(data, "{\"type\":\"siren\",\"uuid\":\"dddd%02x%02x%02x%02x%02x%02x0000000000000004\",\"mac\":\"%02x%02x%02x%02x%02x%02x\",\"clientID\":\"%s\"}",
                new_bluetooth_device_uuid[2], new_bluetooth_device_uuid[3], new_bluetooth_device_uuid[4], new_bluetooth_device_uuid[5],
                new_bluetooth_device_uuid[6], new_bluetooth_device_uuid[7], new_bluetooth_device_mac[0],
                new_bluetooth_device_mac[1], new_bluetooth_device_mac[2], new_bluetooth_device_mac[3],
                new_bluetooth_device_mac[4], new_bluetooth_device_mac[5], output);
    ...
    case ESP_BLE_MESH_CFG_CLIENT_SET_STATE_EVT:
    {
        ...
        case ESP_BLE_MESH_MODEL_OP_APP_KEY_ADD:
        {
            if (device_type == BUTTON)
            {
                ...
                // close
                nvs_close(my_handle);
                sprintf(button_pub, "{\"name\":\"button\",\"clientID\":\"%s\",\"address\":\"0x%04x\"}",
                        output, set_state.model_app_bind.element_addr);
                mqtt_client_publish("/device/button", 1, button_pub);
                // check_node_in_list(set_state.model_app_bind.element_addr);
                break;
            }
            else if (device_type == RGB)
            {
            ...
            else if (device_type == SENSOR)
            {
                ...
                // close
                nvs_close(my_handle);
                sprintf(sensor_pub, "{\"name\":\"sensor\",\"clientID\":\"%s\",\"address\":\"0x%04x\"}",
                        output, set_state.model_app_bind.element_addr);
                mqtt_client_publish("/device/sensor", 1, sensor_pub);
                // check_node_in_list(set_state.model_app_bind.element_addr);
                break;
            }
            else if (device_type == SIREN)
            {
                ...
                // close
                nvs_close(my_handle);
                sprintf(siren_pub, "{\"name\":\"siren\",\"clientID\":\"%s\",\"address\":\"0x%04x\"}",
                        output, set_state.model_app_bind.element_addr);
                mqtt_client_publish("/device/siren", 1, siren_pub);
                // check_node_in_list(set_state.model_app_bind.element_addr);
                break;
            }
            else if (device_type == ENCODER)
            {
                ...
                // close
                nvs_close(my_handle);
                sprintf(encoder_pub, "{\"name\":\"encoder\",\"clientID\":\"%s\",\"address\":\"0x%04x\"}",
                        output, set_state.model_app_bind.element_addr);
                mqtt_client_publish("/device/encoder", 1, encoder_pub);
                // check_node_in_list(set_state.model_app_bind.element_addr);
                break;
            }
        }
            // publish immediately afer app_bind
            // device will resend something for gateway
        case ESP_BLE_MESH_MODEL_OP_MODEL_APP_BIND:
        {     

            if(param->status_cb.model_app_status.model_id== ESP_BLE_MESH_CUSTOM_RGB_MODEL_ID_SERVER)
            {
                 // Open
                printf("\n");
                printf("Opening Non-Volatile Storage (NVS) handle... IN APP BIND ");
                err = nvs_open("storage", NVS_READONLY, &my_handle);
                if (err != ESP_OK)
                {
                    printf("Error (%s) opening NVS handle!\n", esp_err_to_name(err));
                ...
                // close
                nvs_close(my_handle);
                sprintf(rgb_pub, "{\"name\":\"rgb\",\"clientID\":\"%s\",\"address\":\"0x%04x\"}",
                        output, param->params->ctx.addr);
                mqtt_client_publish("/device/rgb", 1, rgb_pub);
        ...
        {      
            if(param->status_cb.model_app_status.model_id == ESP_BLE_MESH_CUSTOM_RGB_MODEL_ID_SERVER){
               char data[100];
                // Open
                printf("\n");
                printf("Opening Non-Volatile Storage (NVS) handle...UNBIND ");
                err = nvs_open("storage", NVS_READONLY, &my_handle);
                if (err != ESP_OK)
                {
                    printf("Error (%s) opening NVS handle!\n", esp_err_to_name(err));
                ...
                // close
                nvs_close(my_handle);
                sprintf(data, "{\"type\":\"rgb\",\"clientID\":\"%s\",\"address\":\"0x%04x\"}",
                        output, param->status_cb.model_app_status.element_addr);
                       // param->status_cb.model_sub_status.sub_addr //group
                mqtt_client_publish("/device/deletedeviceres", 1, data); // nap lai di 
            }
                break;
        }
        case ESP_BLE_MESH_MODEL_OP_MODEL_SUB_ADD:
        {
            ...
            if(param->status_cb.model_app_status.model_id == ESP_BLE_MESH_CUSTOM_RGB_MODEL_ID_SERVER){
               char data[100];
                // Open
                printf("\n");
                printf("Opening Non-Volatile Storage (NVS) handle...SUB_ADD_RGB ");
                err = nvs_open("storage", NVS_READONLY, &my_handle);
                if (err != ESP_OK)
                {
                    printf("Error (%s) opening NVS handle!\n", esp_err_to_name(err));
                ...
                // close
                nvs_close(my_handle);
                sprintf(data, "{\"type\":\"rgb\",\"clientID\":\"%s\",\"group\":\"0x%04x\",\"address\":\"0x%04x\"}",
                        output,param->status_cb.model_sub_status.sub_addr, param->status_cb.model_app_status.element_addr);
                       // param->status_cb.model_sub_status.sub_addr //group
                mqtt_client_publish("/device/registerroom", 1, data); // nap lai di 
            }
            if(param->status_cb.model_app_status.model_id == ESP_BLE_MESH_MODEL_ID_GEN_ONOFF_SRV){
               char data[100];
                // Open
                printf("\n");
                printf("Opening Non-Volatile Storage (NVS) handle...SUB_ADD_BUTTON ");
                err = nvs_open("storage", NVS_READONLY, &my_handle);
                if (err != ESP_OK)
                {
                    printf("Error (%s) opening NVS handle!\n", esp_err_to_name(err));
                ...
                // close
                nvs_close(my_handle);
                sprintf(data, "{\"type\":\"button\",\"clientID\":\"%s\",\"group\":\"0x%04x\",\"address\":\"0x%04x\"}",
                        output,param->status_cb.model_sub_status.sub_addr, param->status_cb.model_app_status.element_addr);
                       // param->status_cb.model_sub_status.sub_addr //group
                mqtt_client_publish("/device/registerroom", 1, data); // nap lai di 
            }
            if(param->status_cb.model_app_status.model_id == ESP_BLE_MESH_CUSTOM_SENSOR_MODEL_ID_SERVER){
               char data[100];
                // Open
                printf("\n");
                printf("Opening Non-Volatile Storage (NVS) handle...SUB_ADD_SIREN ");
                err = nvs_open("storage", NVS_READONLY, &my_handle);
                if (err != ESP_OK)
                {
                    printf("Error (%s) opening NVS handle!\n", esp_err_to_name(err));
                ...
                // close
                nvs_close(my_handle);
                sprintf(data, "{\"type\":\"siren\",\"clientID\":\"%s\",\"group\":\"0x%04x\",\"address\":\"0x%04x\"}",
                        output,param->status_cb.model_sub_status.sub_addr, param->status_cb.model_app_status.element_addr);
                       // param->status_cb.model_sub_status.sub_addr //group
                mqtt_client_publish("/device/registerroom", 1, data); // nap lai di 
            }
        ...
        case ESP_BLE_MESH_MODEL_OP_MODEL_SUB_DELETE:
        {
            if(param->status_cb.model_app_status.model_id == ESP_BLE_MESH_CUSTOM_RGB_MODEL_ID_SERVER){
               char data[100];
                // Open
                printf("\n");
                printf("Opening Non-Volatile Storage (NVS) handle...REMV_RGB ");
                err = nvs_open("storage", NVS_READONLY, &my_handle);
                if (err != ESP_OK)
                {
                    printf("Error (%s) opening NVS handle!\n", esp_err_to_name(err));
                ...
                // close
                nvs_close(my_handle);
                sprintf(data, "{\"type\":\"rgb\",\"clientID\":\"%s\",\"group\":\"%04x\",\"address\":\"0x%04x\"}",
                        output,param->status_cb.model_sub_status.sub_addr, param->status_cb.model_app_status.element_addr);
                       // param->status_cb.model_sub_status.sub_addr //group
                mqtt_client_publish("/device/deleteroom", 1, data); // nap lai di 
            }
            if(param->status_cb.model_app_status.model_id == ESP_BLE_MESH_MODEL_ID_GEN_ONOFF_SRV){
               char data[100];
                // Open
                ...
                // close
                nvs_close(my_handle);
                sprintf(data, "{\"type\":\"button\",\"clientID\":\"%s\",\"group\":\"%04x\",\"address\":\"0x%04x\"}",
                        output,param->status_cb.model_sub_status.sub_addr, param->status_cb.model_app_status.element_addr);
                       // param->status_cb.model_sub_status.sub_addr //group
                mqtt_client_publish("/device/deleteroom", 1, data); // nap lai di 
            }
            if(param->status_cb.model_app_status.model_id == ESP_BLE_MESH_CUSTOM_SENSOR_MODEL_ID_SERVER){
               char data[100];
                // Open
                printf("\n");
                printf("Opening Non-Volatile Storage (NVS) handle...REMV_SIREN ");
                err = nvs_open("storage", NVS_READONLY, &my_handle);
                if (err != ESP_OK)
                {
                    printf("Error (%s) opening NVS handle!\n", esp_err_to_name(err));
                ...
                // close
                nvs_close(my_handle);
                sprintf(data, "{\"type\":\"siren\",\"clientID\":\"%s\",\"group\":\"%04x\",\"address\":\"0x%04x\"}",
                        output,param->status_cb.model_sub_status.sub_addr, param->status_cb.model_app_status.element_addr);
                       // param->status_cb.model_sub_status.sub_addr //group
                mqtt_client_publish("/device/deleteroom", 1, data); // nap lai di 
            }
...
void subscribe_group_address_for_rgb(uint16_t unicast_address, uint16_t group_address)
{
    esp_err_t err;
    esp_ble_mesh_client_common_param_t common;
    ESP_LOGI(TAG, "SUB for RGB");
...
void subscribe_group_address_for_button(uint16_t unicast_address, uint16_t group_address)
{
    esp_err_t err;
    esp_ble_mesh_client_common_param_t common;
    ESP_LOGI(TAG, "SUB for BUTTON");
    common.opcode = ESP_BLE_MESH_MODEL_OP_MODEL_SUB_ADD;
    common.model = config_client.model;
    common.ctx.net_idx = prov_key.net_idx;
    common.ctx.app_idx = prov_key.app_idx;
...
void subscribe_group_address_for_siren(uint16_t unicast_address, uint16_t group_address)
{
    esp_err_t err;
    esp_ble_mesh_client_common_param_t common;
    ESP_LOGI(TAG, "SUB for Siren");
    common.opcode = ESP_BLE_MESH_MODEL_OP_MODEL_SUB_ADD;
    common.model = config_client.model;
    common.ctx.net_idx = prov_key.net_idx;
    common.ctx.app_idx = prov_key.app_idx;
    ...
    esp_ble_mesh_cfg_client_set_state_t set_state = {0};
    // example_ble_mesh_set_msg_common(&common, node, config_client.model, ESP_BLE_MESH_MODEL_OP_APP_KEY_ADD);
    set_state.model_sub_add.model_id = ESP_BLE_MESH_CUSTOM_SENSOR_MODEL_ID_SERVER;
    set_state.model_sub_add.element_addr = unicast_address;
    set_state.model_sub_add.sub_addr = group_address;
    set_state.model_sub_add.company_id = CID_ESP;
    err = esp_ble_mesh_config_client_set_state(&common, &set_state);
...
void delete_subscribe_group_address_for_button(uint16_t unicast_address, uint16_t group_address)
{
    esp_err_t err;
    esp_ble_mesh_client_common_param_t common;
    ESP_LOGI(TAG, "SUB for RGB");
...
void delete_subscribe_group_address_for_rgb(uint16_t unicast_address, uint16_t group_address)
{
    esp_err_t err;
    esp_ble_mesh_client_common_param_t common;
    ESP_LOGI(TAG, "Remove RGB out of the group");
    common.opcode = ESP_BLE_MESH_MODEL_OP_MODEL_SUB_DELETE;
    common.model = config_client.model;
    common.ctx.net_idx = prov_key.net_idx;
    common.ctx.app_idx = prov_key.app_idx;
...
void delete_subscribe_group_address_for_siren(uint16_t unicast_address, uint16_t group_address)
{
    
    esp_err_t err;
    esp_ble_mesh_client_common_param_t common;
    ESP_LOGI(TAG, "Remove SIREN out of the group");
    common.opcode = ESP_BLE_MESH_MODEL_OP_MODEL_SUB_DELETE;
    common.model = config_client.model;
    common.ctx.net_idx = prov_key.net_idx;
    common.ctx.app_idx = prov_key.app_idx;
    ...
    esp_ble_mesh_cfg_client_set_state_t set_state = {0};
    // example_ble_mesh_set_msg_common(&common, node, config_client.model, ESP_BLE_MESH_MODEL_OP_APP_KEY_ADD);
    set_state.model_sub_delete.model_id = ESP_BLE_MESH_CUSTOM_SENSOR_MODEL_ID_SERVER;
    set_state.model_sub_delete.element_addr = unicast_address;
    set_state.model_sub_delete.sub_addr = group_address;
    set_state.model_sub_delete.company_id = CID_ESP;
...
void check_node_in_list(uint16_t unicast_address)
{
    esp_ble_mesh_node_t *node = esp_ble_mesh_provisioner_get_node_with_addr(unicast_address);
    if (node == NULL)
    {
        ESP_LOGI(TAG, "Node with address 0x%04x is not in the list", unicast_address);
    }
    else
    {
        ESP_LOGI(TAG, "Node with address 0x%04x is in the list", unicast_address);
        // Có thể in thêm thông tin chi tiết về node
        // ESP_LOGI(TAG, "Node UUID: %s", esp_log_buffer_hex(TAG, node->uuid, 16));
    }
}
...
void delete_node(uint16_t unicast_address)
{
    ...
    // Tôi có thử hàm này nhưng mà không được. ô thử test thử xem.
    esp_ble_mesh_provisioner_delete_node_with_addr(unicast_address);
...
void delete_rgb(uint16_t unicast_address)
{
    esp_err_t err;
    esp_ble_mesh_client_common_param_t common;
    ESP_LOGI(TAG, "delete RGB");
    common.opcode = ESP_BLE_MESH_MODEL_OP_MODEL_APP_UNBIND;
    common.model = config_client.model;
    common.ctx.net_idx = prov_key.net_idx;
    common.ctx.app_idx = prov_key.app_idx;
    common.ctx.addr = unicast_address;
    ...
    esp_ble_mesh_cfg_client_set_state_t set_state = {0};
    // example_ble_mesh_set_msg_common(&common, node, config_client.model, ESP_BLE_MESH_MODEL_OP_APP_KEY_ADD);
    set_state.model_app_unbind.element_addr = unicast_address;
    set_state.model_app_unbind.model_app_idx = prov_key.app_idx;
    set_state.model_app_unbind.model_id = ESP_BLE_MESH_CUSTOM_RGB_MODEL_ID_SERVER;
    set_state.model_app_unbind.company_id = CID_ESP;
...
void delete_siren(uint16_t unicast_address)
{
    esp_err_t err;
    esp_ble_mesh_client_common_param_t common;
    ESP_LOGI(TAG, "SUB for SIREN");
    ...
    set_state.model_app_unbind.element_addr = unicast_address;
    set_state.model_app_unbind.model_app_idx = prov_key.app_idx;
    set_state.model_app_unbind.model_id = ESP_BLE_MESH_CUSTOM_SENSOR_MODEL_ID_SERVER;
    set_state.model_app_unbind.company_id = CID_ESP;
    ...
    // close
    nvs_close(my_handle);
    sprintf(data, "{\"type\":\"siren\",\"clientID\":\"%s\",\"address\":\"0x%04x\"}",
            output, unicast_address);
    mqtt_client_publish("/device/deletedeviceres", 1, data);
}
void delete_encoder(uint16_t unicast_address)
{
    esp_err_t err;
    esp_ble_mesh_client_common_param_t common;
    ESP_LOGI(TAG, "SUB for encoder");
    common.opcode = ESP_BLE_MESH_MODEL_OP_MODEL_APP_UNBIND;
    common.model = config_client.model;
    common.ctx.net_idx = prov_key.net_idx;
    common.ctx.app_idx = prov_key.app_idx;
    ...
    set_state.model_app_unbind.element_addr = unicast_address;
    set_state.model_app_unbind.model_app_idx = prov_key.app_idx;
    set_state.model_app_unbind.model_id = ESP_BLE_MESH_CUSTOM_ENCODER_MODEL_ID_CLIENT;
    set_state.model_app_unbind.company_id = CID_ESP;
    ...
    // close
    nvs_close(my_handle);
    sprintf(data, "{\"type\":\"encoder\",\"clientID\":\"%s\",\"address\":\"0x%04x\"}",
            output, unicast_address);
    mqtt_client_publish("/device/deletedeviceres", 1, data);
}
void delete_sensor(uint16_t unicast_address)
{
    esp_err_t err;
    esp_ble_mesh_client_common_param_t common;
    ESP_LOGI(TAG, "SUB for sensor");
    common.opcode = ESP_BLE_MESH_MODEL_OP_MODEL_APP_UNBIND;
    common.model = config_client.model;
    common.ctx.net_idx = prov_key.net_idx;
    common.ctx.app_idx = prov_key.app_idx;
    ...
    set_state.model_app_unbind.element_addr = unicast_address;
    set_state.model_app_unbind.model_app_idx = prov_key.app_idx;
    set_state.model_app_unbind.model_id = ESP_BLE_MESH_CUSTOM_SENSOR_MODEL_ID_CLIENT;
    set_state.model_app_unbind.company_id = CID_ESP;
    ...
    // close
    nvs_close(my_handle);
    sprintf(data, "{\"type\":\"sensor\",\"clientID\":\"%s\",\"address\":\"0x%04x\"}",
            output, unicast_address);
    mqtt_client_publish("/device/deletedeviceres", 1, data);
}
void delete_button(uint16_t unicast_address)
{
    esp_err_t err;
    esp_ble_mesh_client_common_param_t common;
    ESP_LOGI(TAG, "SUB for sensor");
    ...
    set_state.model_app_unbind.element_addr = unicast_address;
    set_state.model_app_unbind.model_app_idx = prov_key.app_idx;
    set_state.model_app_unbind.model_id = ESP_BLE_MESH_MODEL_ID_GEN_ONOFF_SRV;
    set_state.model_app_unbind.company_id = ESP_BLE_MESH_CID_NVAL;
    ...
    // close
    nvs_close(my_handle);
    sprintf(data, "{\"type\":\"button\",\"clientID\":\"%s\",\"address\":\"0x%04x\"}",
            output, unicast_address);
    mqtt_client_publish("/device/deletedeviceres", 1, data);
}
