...
void start_esp_local_ctrl_service(void)
{
    ...
    ESP_ERROR_CHECK(esp_local_ctrl_add_property(&on_off));

#elif CONFIG_LOCAL_CONTROL_TEMPERATURE_SENSOR_DEVICE
    /* Create temperature property */
    int32_t *temperature_data = malloc(sizeof(int32_t));
    assert(temperature_data != NULL);
    *temperature_data = 24;
    esp_local_ctrl_prop_t temperature = {
        .name        = "temperature (oC)",
    ...
    /* Create period property */
    int32_t *period_data = malloc(sizeof(int32_t));
    *period_data = 1000;
    esp_local_ctrl_prop_t period = {
        .name        = "period (ms)",
        .type        = PROP_TYPE_INT32,
        .size        = sizeof(int32_t),
        .flags       = 0,
    ...
    // /* Create lower_limit_temp property */
    // int32_t *lower_limit_temp_data = malloc(sizeof(int32_t));
    // *lower_limit_temp_data = 0;
    // esp_local_ctrl_prop_t lower_limit_temp = {
    //     .name        = "Lower limit temp (oC)",
    //     .type        = PROP_TYPE_INT32,
    //     .size        = sizeof(int32_t),
    //     .flags       = 0,
    //     .ctx         = lower_limit_temp_data,
    //     .ctx_free_fn = free
    // };

    // /* Create higher_limit_temp property */
    // int32_t *higher_limit_temp_data = malloc(sizeof(int32_t));
    // *higher_limit_temp_data = 100;
    // esp_local_ctrl_prop_t higher_limit_temp = {
    //     .name        = "Lower limit temp (oC)",
    //     .type        = PROP_TYPE_INT32,
    //     .size        = sizeof(int32_t),
    //     .flags       = 0,
    //     .ctx         = higher_limit_temp_data,
    //     .ctx_free_fn = free
    // };

    /* Register the properties */
    ESP_ERROR_CHECK(esp_local_ctrl_add_property(&temperature));
    ESP_ERROR_CHECK(esp_local_ctrl_add_property(&period));
    // ESP_ERROR_CHECK(esp_local_ctrl_add_property(&lower_limit_temp));
    // ESP_ERROR_CHECK(esp_local_ctrl_add_property(&higher_limit_temp));
    ESP_LOGI(TAG, "esp_local_ctrl service addded temperature and period");
