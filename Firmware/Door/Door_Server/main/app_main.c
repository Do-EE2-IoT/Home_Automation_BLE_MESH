...
SemaphoreHandle_t Get_True_password_sem;
...
void app_main(void)
{
    ...
    Get_True_password_sem = xSemaphoreCreateBinary();
    ...
    xTaskCreate(Task_B_number,"B_number_entered",4096,NULL,3,&B_number_task_handle);
    xTaskCreate(task_for_security, "security task", 4096,NULL,2,&security_task_handle);
    xTaskCreate(Task_accept_change_block_time,"task accept change block time",4096,NULL,3,&Accept_change_block_time_task_handle);
    xTaskCreate(Task_C_number,"task C number",8192,NULL,3,&C_number_task_handle);
    xTaskCreate(Task_accept_change_password, "task accept change password",4096,NULL,3,&accept_change_password_task_handle);
    xTaskCreate(Task_for_Open_Door, "task_for_open_door",8192, NULL,3, &Open_door_task_handle);
    xTaskCreate(Task_Unreal_password, "Unreal",2048,NULL,1,&Unreal_password_handle);
    xTaskCreate(Task_for_lcd_enter_password,"task_lcd",8192 ,NULL,2,&lcd_dislay_password_task_handle); 
    xTaskCreate(nfc_task, "nfc_task", 8192, NULL, 2, &RFID_card_task_handle);
...
void Task_for_lcd_enter_password(void *parameter){
    ...
    if(get_character_4 == '#'){     
        xSemaphoreGive(Get_True_password_sem);
        vTaskDelay(100);                                 
...
void Task_Unreal_password(void *parameter){
     ...
     while(1){
        ...
        xSemaphoreTake(Get_True_password_sem, portMAX_DELAY);
        vTaskSuspend(RFID_card_task_handle);
        vTaskSuspend(lcd_dislay_password_task_handle);
...
void Task_accept_change_block_time(void * parameter){
  ...
  while(1){
    ...
    // tương tự các phần trên, số cần n
    //hập nằm trong khoảng từ ô 00 - 99
    get_character_1 = check_key_pad_col_1();
    get_character_2 = check_key_pad_col_2();
    get_character_3 = check_key_pad_col_3();
    get_character_4 = check_key_pad_col_4();
