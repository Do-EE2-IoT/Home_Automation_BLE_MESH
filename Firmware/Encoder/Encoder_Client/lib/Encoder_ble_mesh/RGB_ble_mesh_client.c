        ...
        switch (param->ctx.recv_op) {
            ...
            case ESP_BLE_MESH_MODEL_OP_MODEL_APP_BIND:
                ...
                if (param->value.state_change.mod_app_bind.company_id == CID_ESP &&
                    param->value.state_change.mod_app_bind.model_id == ESP_BLE_MESH_CUSTOM_ENCODER_MODEL_ID_CLIENT) {
...
esp_err_t ble_mesh_device_init_client(void) {
    ...
    dev_uuid[15] = 0x02;
    ble_mesh_get_dev_uuid(dev_uuid); // keej
...
// 0xC000
esp_err_t ble_mesh_custom_sensor_client_model_message_set(model_encoder_data_t set_data) {
...
esp_err_t ble_mesh_custom_sensor_client_model_message_get(void) {
    ...
    opcode = ESP_BLE_MESH_CUSTOM_RGB_MODEL_OP_GET;
    ...
    ESP_LOGI(TAG, "\t app_idx: 0x%04x, net_idx: 0x%04x, dest_addr: 0x%04x, opcode: 0x%08" PRIx32, 
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Fail to send get message ");
    }
